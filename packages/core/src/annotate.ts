...
class AnnotateImage {
  ...
  constructor(
    ...
    if (elementOrSelector) {
      this.attachToElement(elementOrSelector)
    }
  }
  ...
  private renderExistingPath(
    ...
    if (focusStyles) {
      if (index === this.choseIndex) {
        Object.assign(this.$ctx, focusStyles)
      }
    }
    else {
    ...
    if (type !== 'point' && stroke) {
      this.$ctx.stroke()
    }
    if (!this.$options.readonly && type === 'polygon') {
      this.drawPointIndicators(points)
    }
    if (!focusStyles) {
  ...
  private renderPaths(movePoint?: Point, notClear?: boolean) {
    ...
    if (this.drawing) {
      this.renderNewPath(this.newPath!, movePoint)
    }
  }
  ...
  private renderPathsWithDragPoint(circlePoint?: Point) {
    this.renderPaths()
    if (circlePoint) {
      this.renderDragPoint(circlePoint)
    }
  }
  ...
  private validateActivePathType() {
    ...
    if (
      ...
      this.curSingleType = currentType
    }
    else {
      this.curSingleType = ''
    }

    if (this.curSingleType) {
      this.deselectPath()
    }
  }
  ...
  private focusOnPath(point: Point) {
    ...
    if (!(this.$options.ignoreInvalidSelect && choseIndex === -1)) {
  ...
  private onCanvasMouseUp(e: MouseEvent) {
    ...
    if (this.modifying) {
      ...
      if (!checkPointsEqual(originStartPoint, endPoint)) {
  ...
  private onCanvasMouseDown(e: MouseEvent) {
    ...
    if (!this.$options.allowTypes.includes(type))
      return
  ...
  private modifySelectedPath(e: MouseEvent) {
    ...
    if (pointIndex >= 0) {
      ...
      if (isRect) {
        rectPointsMove(pointIndex)
      }
      else {
        pointMove(points[pointIndex])
      }
  ...
  private getCursorPosition(
    ...
    if (type === 'rect' || type === 'polygon') {
      ...
      if (result) {
        this.operateCursor = { pathType: 'circle', pathIndex: idx }
      }
    }
  ...
  private registerEventHandlers(readonly?: boolean) {
    if (this.$canvas) {
      ...
      this.$canvas.addEventListener('contextmenu', e => this.onCanvasClick(e))
  ...
  private addPath() {
    ...
    if (!singleType) {
      this.choosePath(reverse ? 0 : this.paths.length - 1)
    }
    this.clearCurrentPath()
  }
  ...
  private invertSelectedPath() {
    ...
    if (idx >= 0) {
      this.paths[idx].inner = !this.paths[idx].inner
    }
  ...
  resetOptions(options: PartialAnnotateOptions) {
    ...
    if (this.$options.autoFit !== oldAutoFit) {
